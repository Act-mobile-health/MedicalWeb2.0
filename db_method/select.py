...
def getUserDetailedInfo(D_id):
    ...
    try:
        ...
        value.append(obj.skincolor)
        print value
        keys = ['D_id','name', 'sex', 'birthday','userName','cellphone','weChat','mail', 'title', 'hospital','department','userGroup','registerDate','skincolor']
...
def filterIdByMedicalVisit(lower, upper, start=None, end=None):
    try:
        result = []
        print lower,upper
        for i in MedicalVisit.objects.raw('select distinct P_id as id from Website_medicalvisit where o_time + e_time + h_time >= ' + lower +
                                      ' and o_time + e_time + h_time <= ' + upper + ';'):
            result.append(i)
        print result
        print type(result)
        return result
...
def filterIdByHospitalTimes(lower, upper, start=None, end=None):
    ...
    start = datetime.date(end.year - 1, end.month, end.day)
    try:
        result = MedicalVisit.objects.filter(
...
def getSkinColor(D_id):
    message = {}
    try:
        obj = SkinType.objects.get(id=D_id)
        value = []
        value.append(obj.id)
        value.append(obj.color)
        print value
        keys = ['D_id','color']
        message = tools.dictPackage(keys, value)
    except Exception, e:
        tools.exceptionRecord('select.py','getSkinColor',e)
        pass

    return message

#获取所有病人所有的门诊的个数
def getAllOutPatientServiceInfos():
    q = []
    try:
        values=OutPatientServiceInfo.objects.filter().values('id', 'date', 'place')
        q = list(values)
        print q
        q = sorted(q, key=lambda q:q['date'])
        for a in q:
        ...
        tools.exceptionRecord('select.py', 'getAllOutPatientServiceInfos', e)

    return q


#获取所有病人所有的急诊的个数
def getAllEmergCallInfos():
    q = []
    try:
        values=EmergCallInfo.objects.filter().values('id', 'date', 'place')
        q = list(values)
        print q
        q = sorted(q, key=lambda q:q['date'])
        for a in q:
        ...
        tools.exceptionRecord('select.py', 'getAllEmergCallInfos', e)

    return q


#获取所有病人所有的住院的个数
def getAllInHospitalInfos():
    q = []
    try:
        values=InHospitalInfo.objects.filter().values('id', 'date', 'place')
        q = list(values)
        print q
        q = sorted(q, key=lambda q:q['date'])
        for a in q:
        ...
        tools.exceptionRecord('select.py','getAllInHospitalInfos', e)
