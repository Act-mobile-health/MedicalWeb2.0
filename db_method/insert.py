...
def addPatientInfo(data):
    try:
        ...
        newObj = PatientInfo(P_id = data['P_id'], sign = data['sign'], name = data['name'], sex = data['sex'],IDCardNum = data['IDCardNum'],
                                 birthday = d, age = age, nation = data['nation'], height = data['height'],
...
# @0922
def addPatientApp(data):
    try:
        print data
        values = PatientInfo.objects.filter(P_id = data['P_id'])
        if len(list(values)) > 0:
            return -1
        obj = PatientInfo(P_id = data['P_id'], name = data['name'], sex = data['sex'], age = int(data['age']),
                          cellphone = data['cellphone'], birthday = datetime.datetime.strptime("1970-01-01", "%Y-%m-%d").date())
        obj.save()
        newM = MedicalVisit(P_id=data['P_id'],o_time=0,e_time=0,h_time=0)
        newM.save()
        return 0
    except Exception, e:
        tools.exceptionRecord('insert.py','addPatientApp',e)
        return -1