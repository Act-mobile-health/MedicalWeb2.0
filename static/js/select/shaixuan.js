...
oSelectList.onclick = function(e,a){
    ...
    if (tagName == 'INPUT') {
        console.log('input');
        console.log(tag)
        console.log(tag.getAttribute('type').toUpperCase());
        if (tag.getAttribute('type').toUpperCase() == 'TEXT') {
            var val = next(tag);
            console.log(val);
            console.log(parents(tag,'dl'))
            console.log(parents(tag,'dl').getAttribute('attr'))
            if (tag.checked) {
...
function xxx(e, a) {
    ...
    if (tagName == 'A') { //如果点击 的是 A
        ...
        if (!oPrevInput) { //如果上一个节点没有则认为点击的是 '不限'
            ...
            if (trim(prev(parent).innerHTML) == '就医次数') { //这里是直接根据 text来比较的.建议加个自定义属性作标识符
                console.log("就医次数")
                for (var i = 0; i < aRadio.length; i++) {
                    aRadio[i].checked = false;
                }
    ...
    if(a && a == 2) {
        ...
        console.log('aaa')
       if (radioVal) infor += '<div class=\"selectedInfor selectedShow\"><span>就医次数</span><label>' + radioVal + '</label><em p="2"></em></div>';
    }
